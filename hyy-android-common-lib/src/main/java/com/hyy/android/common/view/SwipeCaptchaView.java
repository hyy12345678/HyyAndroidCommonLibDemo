...
import com.hyy.android.common.util.DrawHelperUtils;
...
public class SwipeCaptchaView extends AppCompatImageView {
    ...
    private int mSuccessAnimOfffset;//动画的offset
    private Path mSuccessPath;//成功动画 平行四边形Path


    public SwipeCaptchaView(@NonNull Context context) {
        this(context, null);
    }

    public SwipeCaptchaView(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SwipeCaptchaView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs, defStyleAttr);
    }

    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        Log.e(TAG, "-----------SwipeCaptchaView init invoked--------------");
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.SwipeCaptchaView_captchaHeight) {
                ...
                mMatchDeviation = ta.getDimension(attr, defaultSize);
            }
        }
        ta.recycle();
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        Log.e(TAG, "-----------SwipeCaptchaView onSizeChanged invoked--------------");
        mWidth = w;
        mHeight = h;
        //动画区域 会用到宽高
        createMatchAnim();
    ...
    private void createMatchAnim() {
        ...
        mSuccessAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                mSuccessAnimOfffset = (int) animation.getAnimatedValue();
                invalidate();
            }
        });
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isMatchMode) {
            ...
            if (isShowSuccessAnim) {
                canvas.translate(mSuccessAnimOfffset, 0);
